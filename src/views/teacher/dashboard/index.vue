...
const stats = computed(() => {
  return {
    studentCount: dashboardData.value?.studentCount || 0,
    classAverageScore: dashboardData.value?.classAverageScore || 0,
    classAvgActivityScore: dashboardData.value?.classAvgActivityScore || 0,
    passRate: dashboardData.value?.passRate || 0,
  }
})
...
const fetchClassList = async () => {
  ...
  if (res && res.data) {
    ...
    // 设置默认班级，由 watch 自动触发主数据加载
    if (res.data.length && !searchModel.value.classId) {
...
const fetchAiAnalysis = async (forceRefresh = false) => {
  ...
  if (!forceRefresh && aiFetched.value) return
...
// 班级切换时重新加载主数据，AI在后台独立加载
watch(() => searchModel.value.classId, async (newClassId) => {
  if (newClassId) {
...
const fetchDashboardData = async () => {
  if (!searchModel.value.classId) return
  const res = await tDashboardApi.getDashboardData(searchModel.value.classId, searchModel.value.courseId)
  if (res && res.data) {
...
const fetchMoreData = async () => {
  const res = await tDashboardApi.getMoreData(searchModel.value.classId, searchModel.value.courseId)
  if (res && res.data) {
...
const initGradeChart = async () => {
  const dist = dashboardData.value?.scoreDistribution
  if (!dist) {
    gradeEchartData.value = {
      title: { text: '暂无考试数据', left: 'center' },
  ...
  const gradeLevels = {
    '优秀(>=90)': { color: '#67c23a' },
    '良好(80-89)': { color: '#409eff' },
    '中等(70-79)': { color: '#e6a23c' },
    '及格(60-69)': { color: '#f56c6c' },
    '不及格(<60)': { color: '#909399' }
  }

  const categories = Object.keys(dist).map(key => `${key}`)
  const values = Object.values(dist)

  gradeEchartData.value = {
    title: {
      text: `${dashboardData.value?.classInfo?.name || ''} 考试成绩等级分布`,
      left: 'center'
    },
    tooltip: {
      trigger: 'axis',
      ...
      formatter: (params) => {
        ...
        return `${params[0].name}<br/>📊 人数: ${params[0].value} 人<br/>📈 占比: ${total > 0 ? ((params[0].value / total) * 100).toFixed(1) : 0}%`
      }
    },
    grid: {
      left: '10%',
...
const initKnowledgeChart = async () => {
  const data = moredashboardData?.value?.weakKnowledgePoints
  if (!data || data.length === 0) {
    weaknessChartData.value = {
      title: { text: '暂无薄弱知识点数据', left: 'center' },
      graphic: { type: 'text', left: 'center', top: 'center', style: { text: '暂无数据', fill: '#999' } }
    }
    return
  }
  const knowledgePoints = data.map(item => item.knowledgePointName)
  const rates = data.map(item => item.avgMastery)
  if (knowledgeView.value === 'bar') {
    weaknessChartData.value = {
      title: { text: '薄弱知识点（按平均得分升序）', left: 'center', textStyle: { fontSize: 14 } },
      tooltip: {
        trigger: 'axis',
        formatter: (params) => `${params[0].name}<br/>平均得分: ${params[0].value}分`
      },
      grid: { left: '18%', containLabel: true },
      xAxis: { type: 'value', name: '平均得分', max: 100 },
      yAxis: { type: 'category', data: knowledgePoints, axisLabel: { fontSize: 11 } },
      series: [{
        type: 'bar',
        data: rates,
        itemStyle: {
          borderRadius: [0, 8, 8, 0], color: (params) => {
            if (params.value < 60) return '#f56c6c'
    ...
    weaknessChartData.value = {
      title: { text: '知识点掌握度词云', left: 'center', top: 5 },
      tooltip: { trigger: 'item', formatter: (params) => `${params.name}<br/>平均得分: ${params.value}分` },
      series: [{
        type: 'wordCloud',
        shape: 'star',
        width: '90%',
        ...
        textStyle: {
          ...
          color: (value) => {
            const rate = value.data[1]
            if (rate < 60) return '#f56c6c'
            if (rate < 70) return '#e6a23c'
            return '#409eff'
          }
        },
        data: knowledgePoints.map((name, i) => ({ name, value: Math.round(rates[i]) }))
      }]
    }
  }
}
...
const handleExportImage = () => {
  if (exportContentRef.value) {
    exportToImage(exportContentRef.value, '教学看板')
  } else {
...
const handleExportPDF = () => {
  if (exportContentRef.value) {
    exportToPDF(exportContentRef.value, `教学看板`)
  } else {
    ElMessage.warning('没有可导出的内容')
  }
}
  ...
  <div class="dashboard-container">
    ...
    <div ref="exportContentRef" class="export-content">
      <el-row :gutter="20" style="margin-top: 20px;">
        ...
        <el-col :span="6">
          <StatBox icon="fa-chart-line" title="考试平均分" :stat-num="stats.classAverageScore?.toFixed?.(2) ?? 0.00" />
        </el-col>
        <el-col :span="6">
          <StatBox icon="fa-star" title="活跃度平均分" :stat-num="stats.classAvgActivityScore?.toFixed?.(2) ?? 0.00" />
        </el-col>
        <el-col :span="6">
          <StatBox icon="fa-tasks" title="考试及格率" :stat-num="stats.passRate?.toFixed?.(2) ?? 0.00" />
        </el-col>
      </el-row>
      ...
      <el-row style="margin-top: 20px;" v-loading="loading">
        <el-col :span="24">
          <el-card shadow="hover">
            <div class="chart-header">
              <h3>薄弱知识点分析 <span class="subtitle">(按平均得分升序，最低10个)</span></h3>
